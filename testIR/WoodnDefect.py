...
class ColorWoodDetector:
    def __init__(self):
        ...
        # Defect detection model configuration
        self.inference_host_address = "@local"
        self.zoo_url = "/home/inspectura/Desktop/InspecturaGUI/models/V2DefectCombined--640x640_quant_hailort_hailo8_1"
    ...
    def detect_wood_comprehensive(self, image: np.ndarray, profile_names: List[str] = None, roi: Tuple[int, int, int, int] = None, camera: str = 'top') -> Dict:
        ...
        # Step 6: Detect defects in auto ROI if wood detected
        if auto_roi and len(wood_candidates) > 0:
            defects = self.detect_defects_in_roi(image, auto_roi)
            result['defects'] = defects
            result['defect_detected'] = len(defects) > 0
        else:
            result['defects'] = []
    ...
    def visualize_detection(self, image: np.ndarray, detection_result: Dict) -> np.ndarray:
        ...
        if detection_result.get('defect_detected', False) and detection_result['auto_roi']:
            roi_x, roi_y, _, _ = detection_result['auto_roi']
            for defect in detection_result['defects']:
                bbox = defect.get('bbox', [])
                if len(bbox) >= 4:
                    x1, y1, x2, y2 = bbox[:4]
                    cv2.rectangle(vis_image, (roi_x + int(x1), roi_y + int(y1)), (roi_x + int(x2), roi_y + int(y2)), (0, 0, 255), 2)
                    label = defect['label']
                    cv2.putText(vis_image, label, (roi_x + int(x1), roi_y + int(y1) - 5), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
    ...
    def detect_defects_in_roi(self, image: np.ndarray, roi: Tuple[int, int, int, int]) -> List[Dict]:
        """Detect defects in the specified ROI using the DeGirum model"""
        if self.defect_model is None:
            return []
        x, y, w, h = roi
        cropped = image[y:y+h, x:x+w]
        try:
            inference_result = self.defect_model.predict(cropped)
            ...
            for det in detections:
                ...
                if confidence >= self.confidence_threshold:
                    label = det['label']
                    if label in ["Dead Knots", "Knots with Crack", "Live Knots", "Missing Knots"]:
                        defects.append(det)

            # If multiple defects detected, keep only the one with highest confidence
            if len(defects) > 1:
                defects.sort(key=lambda x: x.get('score', 0), reverse=True)
                defects = [defects[0]]  # Keep only the highest confidence defect
            return defects
        except Exception as e:
...
def main():
    ...
    try:
        while True:
            ...
            # Draw defect bounding boxes on frame 0
            if detection_result0.get('defect_detected', False) and detection_result0['auto_roi']:
                roi_x, roi_y, _, _ = detection_result0['auto_roi']
                for defect in detection_result0['defects']:
                    bbox = defect.get('bbox', [])
                    if len(bbox) >= 4:
                        x1, y1, x2, y2 = bbox[:4]
                        cv2.rectangle(frame0, (roi_x + int(x1), roi_y + int(y1)), (roi_x + int(x2), roi_y + int(y2)), (0, 0, 255), 2)
                        label = defect['label']
                        cv2.putText(frame0, label, (roi_x + int(x1), roi_y + int(y1) - 5), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
            ...
            # Draw defect bounding boxes on frame 2
            if bottom_display_enabled and detection_result2.get('defect_detected', False) and detection_result2['auto_roi']:
                roi_x, roi_y, _, _ = detection_result2['auto_roi']
                for defect in detection_result2['defects']:
                    bbox = defect.get('bbox', [])
                    if len(bbox) >= 4:
                        x1, y1, x2, y2 = bbox[:4]
                        cv2.rectangle(frame2_flipped, (roi_x + int(x1), roi_y + int(y1)), (roi_x + int(x2), roi_y + int(y2)), (0, 0, 255), 2)
                        label = defect['label']
                        cv2.putText(frame2_flipped, label, (roi_x + int(x1), roi_y + int(y1) - 5), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
            ...
            if detection_enabled:
                ...
                if wood_detected0 and detection_result0.get('defect_detected', False):
                    status0 += " + DEFECT"
                    color0 = (0, 0, 255)  # Red if defect
            else:
            ...
            if detection_enabled:
                if bottom_display_enabled:
                    ...
                    if wood_detected2 and detection_result2.get('defect_detected', False):
                        status2 += " + DEFECT"
                        color2 = (0, 0, 255)
                else:
            ...
            if frame_count % 30 == 0:
                ...
                defect0_str = ", defect=yes" if detection_result0.get('defect_detected', False) else ""
                defect2_str = ", defect=yes" if detection_result2.get('defect_detected', False) else ""
                print(f"Frame {frame_count}: Cam0={status0}({confidence0:.2f}{width0_str}{defect0_str}), Cam2={status2}({confidence2:.2f}{width2_str}{defect2_str})")
