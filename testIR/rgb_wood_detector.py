...
class ColorWoodDetector:
    def __init__(self):
        ...
        # Detection parameters - Reduced for better sensitivity
        self.min_contour_area = 500       # Reduced for smaller wood pieces
    ...
    def detect_rectangular_contours(self, mask: np.ndarray, camera: str = 'top') -> List[Dict]:
        ...
        for i, contour in enumerate(contours):
            ...
            # Filter by minimum size to prevent small detections - reduced for better sensitivity
            if camera == 'top':
                min_height = 50   # Reduced from 266
                min_width = 30    # Reduced from 100
            elif camera == 'bottom':
                min_height = 50   # Reduced from 286
                min_width = 30    # Reduced from 100
            else:
                min_height = 30   # Reduced from 100
                min_width = 20    # Reduced from 100
